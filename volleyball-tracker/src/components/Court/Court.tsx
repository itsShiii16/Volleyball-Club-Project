...
function usePossession() {
  ...
  return useMemo(() => {
    ...
    if (sendingSkills.includes(skill)) return opponentOf(lastEvent.teamId);
    ...
    <div className="w-full max-w-7xl mx-auto aspect-[4/3] rounded-3xl bg-sky-500 p-2 sm:p-4 xl:p-8 shadow-xl transition-all">
      <div className="relative h-full w-full rounded-2xl border-[4px] sm:border-[6px] xl:border-[8px] border-white/90 bg-amber-400">
        ...
        <div className="absolute inset-1 sm:inset-2 xl:inset-6 grid grid-cols-2 gap-0 z-10">
          <TeamHalf teamId={leftTeam} side="left" hasPossession={possessionTeam === leftTeam} />
          <TeamHalf teamId={rightTeam} side="right" hasPossession={possessionTeam === rightTeam} />
        </div>
      </div>
    </div>
  );
...
function TeamHalf({ side, teamId, hasPossession }: { side: "left" | "right"; teamId: TeamId; hasPossession: boolean }) {
  ...
  const BackCol = (<div className={gridClass}><CourtSlot teamId={teamId} slot={5} /><CourtSlot teamId={teamId} slot={6} /><CourtSlot teamId={teamId} slot={1} /></div>);
  const FrontCol = (<div className={gridClass}><CourtSlot teamId={teamId} slot={4} /><CourtSlot teamId={teamId} slot={3} /><CourtSlot teamId={teamId} slot={2} /></div>);

  return (
    <div className={["h-full w-full border-white/90 flex flex-col justify-center transition-all duration-500", isLeft ? "border-r-[2px] xl:border-r-[4px]" : "border-l-[2px] xl:border-l-[4px]", hasPossession ? "bg-yellow-400/20 shadow-[inset_0_0_60px_rgba(250,204,21,0.4)]" : ""].join(" ")}>
      <div className={["h-full w-full grid grid-cols-2", "gap-1 sm:gap-3 lg:gap-4 xl:gap-8", "py-2 sm:py-4 xl:py-8", "px-1 sm:px-2 xl:px-6"].join(" ")}>
...
function CourtSlot({ teamId, slot }: { teamId: TeamId; slot: RotationSlot }) {
  ...
  const replacedPlayer = isLiberoAutoSub && liberoSwap.replacedPlayerId ? players.find((p) => p.id === liberoSwap.replacedPlayerId) ?? null : null;
  const dropId = `${teamId}-${slot}`;
  const { setNodeRef, isOver } = useDroppable({ id: dropId, data: { teamId, slot } });
  ...
  function handlePrimaryClick() {
    if (!playerId) { selectSlot(teamId, slot, "bench"); return; }
    openScoresheet(teamId, slot);
  }

  return (
    <div ref={setNodeRef} onClick={handlePrimaryClick} className={["w-full h-full max-w-[180px] max-h-[120px] aspect-[16/10]", "rounded-lg sm:rounded-xl xl:rounded-2xl shadow-md transition-all px-2 py-1 sm:px-3 sm:py-2 xl:px-4 xl:py-3 flex flex-col text-left relative", "cursor-pointer select-none border-[2px] sm:border-[3px]", "text-gray-900", playerId ? `${posColors.badgeBg} ${posColors.ring.replace('ring-', 'border-')}` : "bg-gray-50/90 border-white/60", isSelected ? "ring-4 ring-blue-500 z-20 scale-105" : "hover:scale-[1.03] hover:shadow-lg", isOver ? "ring-4 ring-emerald-400 bg-emerald-100" : "", isServer ? "ring-4 ring-yellow-400 shadow-xl shadow-yellow-500/40 z-10" : "", (isServer && !rallyInProgress) ? "animate-pulse" : "", isLiberoAutoSub ? "ring-4 ring-teal-400 shadow-xl shadow-teal-500/40 z-10" : "", isFrontRow ? "border-t-[6px] border-t-orange-400/80" : ""].join(" ")}>
      <div className="flex items-center justify-between leading-none mb-auto">
        <div className={`text-[9px] sm:text-[10px] xl:text-sm uppercase font-black tracking-wider ${isFrontRow ? "text-orange-600" : "opacity-60"}`}>{slotLabel[slot]}</div>
        ...
        {player ? (
          <>
            <div className="flex flex-col items-center">
                <div className="text-2xl sm:text-3xl xl:text-5xl font-black leading-none tracking-tighter text-gray-900 drop-shadow-sm">{player.jerseyNumber}</div>
                <div className="text-xs sm:text-sm xl:text-lg font-bold truncate leading-tight mt-0.5 xl:mt-1 max-w-full px-1">{player.name}</div>
                <div className={`mt-1 hidden sm:block text-[9px] xl:text-xs font-black px-2 py-0.5 rounded-full shadow-sm border border-black/5 ${posColors.chipBg} ${posColors.chipText}`}>{player.position}</div>
            </div>
            {isLiberoAutoSub && replacedPlayer && (<div className="hidden xl:block text-[10px] font-bold text-gray-500 truncate mt-1 bg-white/50 px-1.5 py-0.5 rounded">Sub #{replacedPlayer.jerseyNumber}</div>)}
          </>
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity bg-black/10 rounded-xl backdrop-blur-[1px]">
         <span className="bg-white text-black font-black text-xs px-3 py-1.5 rounded-full shadow-lg uppercase tracking-wider scale-110">{playerId ? "Score" : "Add"}</span>
      </div>
    </div>
  );
}
...
function DashedMarker({ className }: { className?: string }) {
  return (<div className={className}><div className="h-full w-full border-r-[2px] sm:border-r-[3px] border-dashed border-white/50" /></div>);
}