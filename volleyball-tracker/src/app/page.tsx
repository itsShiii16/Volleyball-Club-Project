...
export default function Home() {
  ...
  // Safety: restore scroll on unmount (in case drag gets interrupted)
  useEffect(() => {
  ...
  // Rotate by side (L/R), not by team letter
        ...
        <div className="max-w-6xl mx-auto flex flex-col gap-4">
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-center">
            ...
            <div className="flex gap-2 flex-wrap items-center">
              <button
                ...
                className="px-4 py-2 rounded-xl font-semibold shadow bg-white text-black hover:bg-white/90"
              >
                Set up Roster
              </button>
            ...
            <div className="flex gap-2 flex-wrap items-center justify-end">
              ...
              <button
                onClick={rotateLeftSide}
                className="px-4 py-2 rounded-xl font-semibold shadow bg-white text-black hover:bg-white/90"
                title={`Rotate LEFT side (${leftTeam})`}
              >
                Rotate L
              </button>

              <button
                onClick={rotateRightSide}
                className="px-4 py-2 rounded-xl font-semibold shadow bg-white text-black hover:bg-white/90"
                title={`Rotate RIGHT side (${rightTeam})`}
              >
                Rotate R
              </button>
            </div>
          ...
          <div className="grid grid-cols-[160px_1fr_160px] gap-3 items-center">
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={clearLeftSide}
                className="px-4 py-3 rounded-xl font-semibold shadow bg-white text-black hover:bg-white/90"
              >
                Clear Team
              </button>
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                />
                <button className="px-3 py-2 rounded-lg bg-white shadow text-xs font-bold text-black">
                  Timeout
                </button>
              </div>
            </div>

            {/* Center score + set score */}
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setSetsA((v) => Math.max(0, v - 1))}
                  className="h-9 w-9 rounded-lg bg-white shadow font-black text-black"
                >
                  −
                </button>
                <button
                  onClick={() => setSetsA((v) => Math.min(5, v + 1))}
                  className="h-9 w-9 rounded-lg bg-white shadow font-black text-black"
                >
                  +
                </button>
                ...
                <button
                  onClick={() => setSetsB((v) => Math.max(0, v - 1))}
                  className="h-9 w-9 rounded-lg bg-white shadow font-black text-black"
                >
                  −
                </button>
                <button
                  onClick={() => setSetsB((v) => Math.min(5, v + 1))}
                  className="h-9 w-9 rounded-lg bg-white shadow font-black text-black"
                >
                  +
                </button>
              ...
              <div className="rounded-xl bg-white shadow px-10 py-2 text-black font-black text-lg">
                {scoreA} - {scoreB}
              </div>
              ...
              <button
                ...
                className={[
                  "px-5 py-2 rounded-xl font-bold shadow transition",
                  rosterReady
                    ? "bg-white text-black hover:bg-white/90"
                    : "bg-white/60 text-black/40 cursor-not-allowed",
                ].join(" ")}
    ...
    <div className="flex items-center gap-2">
      {value.map((v, idx) => (
        <button
          ...
          className={["h-4 w-4 rounded-sm shadow", v ? "bg-gray-900" : "bg-white"].join(" ")}
          aria-label={`Timeout ${idx + 1}`}
          title={`Timeout ${idx + 1}`}
        />
      ))}
    </div>
